...
public extension Resource {
	...
	*/
	func asRelativeReference() throws -> Reference {
		...
		reference.reference = FHIRString(path).asPrimitive()
		if let display = preferredRelativeReferenceDisplay() {
			reference.display = FHIRString(display).asPrimitive()
		}
		return reference
	}
	...
	*/
	func relativeURLBase() -> String {
		return type(of: self).resourceType.rawValue
	}
	...
	class func readFrom(_ path: String, server: FHIRServer, options: FHIRRequestOption = [], callback: @escaping FHIRResourceErrorCallback) {
		...
		server.performRequest(against: path, handler: handler) { response in
			if let error = response.error {
				...
				do {
					let resource = try response.responseResource(ofType: self)
					resource._server = server
					try response.applyHeaders(to: resource)
					if nil == resource.id, let lpc = URL(string: path) {
						resource.id = FHIRString(lpc.lastPathComponent).asPrimitive()
					}
		...
		server.performRequest(against: relativeURLBase(), handler: handler) { response in
			if nil == response.error {
				...
				// no resource, but hopefully the id was detected in the Location header, so go and read the resource
					...
					if let id = self.id?.value?.string {
