...
open class PatientListQuery {
	...
	func execute(onServer server: FHIRServer, order: PatientListOrder, callback: @escaping (ModelsR4.Bundle?, FHIRError?) -> Void) {
		if isDone {
			callback(nil, nil)
			return
		}
		
		let cb: (ModelsR4.Bundle?, FHIRError?) -> Void = { bundle, error in
