...
class Auth {
	...
	*/
	...
	/**
	Convenience initializer from the server capability statement's rest.security parts.
	
	- parameter fromCapabilitySecurity: The server capability statement's rest.security pieces to inspect
	...
	*/
	convenience init?(fromCapabilitySecurity security: CapabilityStatementRestSecurity, server: Server, settings: OAuth2JSON?) {
		...
		if let smartauth = security.extensions(for: "http://fhir-registry.smarthealthit.org/StructureDefinition/oauth-uris").first?.`extension` {
			for subext in smartauth where nil != subext.url.value {
				switch subext.url.value?.url.absoluteString {
				case "authorize":
					switch subext.value {
					case .uri(let uri):
//						print("authorize_uri: \(uri.value?.url.absoluteString ?? "<unknown>")")
						authSettings["authorize_uri"] = uri.value?.url.absoluteString
					default: break
					}
				case "token":
					switch subext.value {
					case .uri(let uri):
//						print("token_uri: \(uri.value?.url.absoluteString ?? "<unknown>")")
						authSettings["token_uri"] = uri.value?.url.absoluteString
					default: break
					}
				case "register":
					switch subext.value {
					case .uri(let uri):
//						print("registration_uri: \(uri.value?.url.absoluteString ?? "<unknown>")")
						authSettings["registration_uri"] = uri.value?.url.absoluteString
					default: break
					}
				default:
					break
