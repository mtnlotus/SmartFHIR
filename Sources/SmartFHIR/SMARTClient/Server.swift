...
settings dictionary provided upon `Client` initalization or from the server's capability statement.

The server's capability statement is automatically downloaded the first time it's needed for various tasks, such as instantiating the `Auth`
...
open class Server: FHIROpenServer, OAuth2RequestPerformer {
	...
	/// An optional name of the server; will be read from capability statement unless manually assigned.
...
//		get { return auth?.oauth?.authorizedScope }
//	}
	...
	*/
	...
	override open func didSetCapabilityStatement(_ cabability: CapabilityStatement) {
		if nil == name, let cName = cabability.name?.value?.string {
			name = cName
		}
	...
	*/
	...
	after the capability statement has been fetched.
	*/
